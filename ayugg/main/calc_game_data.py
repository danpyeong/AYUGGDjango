...
def statics_data():
    ...
    tier_dict = {'IRON': {}, 'BRONZE': {}, 'SILVER': {}, 'GOLD': {}, 'PLATINUM': {}, 'EMERALD': {},
                'DIAMOND': {}, 'MASTER': {}, 'GRANDMASTER': {}, 'CHALLENGER': {}}

    data = data_set_champions()
    
    for tier in tier_dict:
        for line in line_dict:
            for d in data:
                data[d]['line'] = str(line)
                data[d]['tier'] = str(tier)
            
            tmp = copy.deepcopy(data)
            tier_dict[tier][line] = tmp

    match_version = { version : {} }

    for ver in match_version:
        match_version[ver] = tier_dict
     
    # 자료구조를 활용해 match_data에 있는 데이터들을 정리
    def input_data(match_version):
        ban_struct = []
        
        # match_version[버전][티어][라인]
        for data in match_data:
            ...
            if data.bans not in ban_struct:
                for ban in data.bans:
                    if str(ban) != 'None':
                        ban_champ_name = champ_data[ban]
                        ban_champ_name['ban'] += 1
                ban_struct.append(data.bans)
            
            play_champ_name['tier'] = data.game_tier
            play_champ_name['line'] = data.line
            
        return match_version

    game_data = input_data(match_version)

    # 이제 각 챔피언의 픽률, 승률, 밴률을 계산함
            ...
            for tier in tier_dict:
                for line in line_dict:
                    for index, champ in enumerate(champ_data):
                        data_ = game_data[data][tier][line][str(champ)]
                        ...
                        if play_total != 0:
                            ...
                            # 필요한 값 : 버전, 라인, 챔피언 이미지, 챔피언 이름, 승률, 픽률, 밴률, 카운터
                            #  - 버전
                            res['version'] = version
                            #  - 라인
                            line = data_['line']
                            ...
                            # match_version[version][tier][line].append(res.copy())
                            match_version[version][tier][line][str(champ)] = res.copy()
